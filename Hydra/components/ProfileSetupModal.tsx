...
export default function ProfileSetupModal({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // 1. Guard against non-UUID / guest submission
        if (!userId || userId === 'guest_user' || userId.startsWith('guest_')) {
            setErrorMsg('You must be logged in to create or update a profile handle.');
            return;
        }
                        ...
                        <div className="flex items-center justify-between gap-2">
                            {AVATAR_PRESETS.map((preset) => (
                                <button
                                    ...
                                    className={`relative rounded-full p-1 transition-all ${avatarUrl === preset
